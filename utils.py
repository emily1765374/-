...
def validate_expense(expense_date, amount, category, place, memo):
    """지출 입력값을 검증하고 정리한다.
...
def build_report_text(period_label, start: date, end: date, rows_asc, category_totals: dict, total: int) -> str:
    """팀장 보고용 텍스트 (PRD 13장). rows_asc는 날짜 오래된 순으로 정렬된 지출 목록."""
    lines = [
...
def build_backup_text(rows_desc, category_totals: dict, total: int, count: int, backup_at: datetime) -> str:
